...
def is_valid_link(href, domain):
    """Перевірка, чи посилання веде саме на оголошення, а не на 'допомогу' чи 'інші міста'"""
    # Список слів-пасток, які ми ігноруємо
    ...
    # Специфічні фільтри для сайтів, щоб не вилітати за межі Харкова
    ...
    for url in URLS:
        try:
            ...
            for a in soup.find_all('a', href=True):
                ...
                # Робимо посилання повним
                if href.startswith('/'):
                    href = f"https://{domain}{href}"
                
                # ФІЛЬТРАЦІЯ:
                # 1. Посилання має бути з того ж домену
                # 2. Воно не має бути в списку вже бачених
                # 3. Воно має пройти наш тест на 'справжнє оголошення'
