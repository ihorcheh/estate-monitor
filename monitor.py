...
# Пряме підключення токенів для гарантії результату
...
def save_id(realty_id):
    with open(DB_FILE, 'a') as f: f.write(f"{realty_id}\n")
...
def check_sites():
    ...
    send_msg(f"✅ Моніторинг завершено. Знайдено: {new_count}")
